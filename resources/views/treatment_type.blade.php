        ...
        <div class="row mb-2">
            ...
            <div class="col-sm-6">
                <ol class="breadcrumb float-sm-right">
                    ...
                    <li class="breadcrumb-item active">Treatment Type</li>
                </ol>
            </div>
        </div>
    </div><!-- /.container-fluid -->
            ...
            <form id="form-treat" method="post" class="form-horizontal" data-toggle="validator" enctype="multipart/form-data">
                ...
                <div class="modal-body">
                    ...
                    <div class="form-group">
                        <label for="nama" class="col-md-3 control-label">Jenis Treatment</label>
                        <div class="col-md-6">
...
<script type="text/javascript">
    ...
    $(document).on('click', '.edit', function() {
        var id = $(this).data('id');
        $.ajax({
            url: "{{url('api/treatment_type/edit')}}" + "/" + id,
            type: "GET",
            headers: header,
            dataType: "JSON",
        ...
        if ($("#form-treat").length > 0) {
            $("#form-treat").validate({
                ...
                submitHandler: function(form) {
                    ...
                    if ($('#saveBtn').val() == 'Add') {
                        url = "{{ route('treattype.store') }}";
                        method = "POST";
                    } else {
                        var id = document.getElementById('id').value;
                        url = "{{url('api/treatment_type/update')}}" + "/" + id;
                        method = "PUT";
                    }
                    $.ajax({
    ...
    $(document).on('click', '.delete', function() {
        Swal.fire({
            ...
            if (result.value) {
                ...
                $.ajax({
                    type: "DELETE",
                    url: "{{ url('api/treatment_type/delete') }}" + '/' + id,
                    headers: header,
                    success: function(data) {
                        var oTable = $('#treat-table').dataTable();
                        oTable.fnDraw(false);
